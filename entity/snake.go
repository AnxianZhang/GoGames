...
type Snake struct {
	...
	direction *geometry.Position // can have an x and y acceleration, the snake will move along this direction
}

func NewSnake(start, direction *geometry.Position) *Snake {
	return &Snake{[]*geometry.Position{start}, direction}
}
...
func (s *Snake) Update(environment generic.WorldView) gameStatus.GameStatus {
	...
	if needToGrow {
		s.body = append([]*geometry.Position{newHeadPosition}, s.body[:len(s.body)]...)
	} else {
		s.body = append([]*geometry.Position{newHeadPosition}, s.body[:len(s.body)-1]...)
	}
...
func (s *Snake) SetDirection(newDirection *geometry.Position) {
